                       ...
                       PPMThread* thread, int numThread, cudaStream_t* streamSmooth, int filtro)
{
   ...
   gpuErrchk( cudaBindTexture2D(NULL,textureIn,GPU_input,width,thread[numThread].linhas,gpu_image_pitch) );
    ...
    kernelTexture<<<grid_size,block_size, 0, streamSmooth[numThread]>>>(GPU_output,width,imageParams->linha,gpu_image_pitch,thread[numThread].lf,thread[numThread].li);
