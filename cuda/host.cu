...
void applySmooth(initialParams* ct, PPMImageParams* imageParams, PPMBlock* block, int numBlock, cudaStream_t* streamSmooth) {
    ...
    if (strcmp(imageParams->tipo, "P6")==0) {
        ...
        dim3 gridDims((unsigned int) (block[numBlock].linhasIn/blockDims.x)+1, 1, 1 );
        ...
        printf("Apply Smooth[%d][%s] - li:%d, lf:%d %d\n",
               numBlock, imageParams->tipo, block[numBlock].linhasIn, block[numBlock].lf, (block[numBlock].linhasIn/blockDims.x));
