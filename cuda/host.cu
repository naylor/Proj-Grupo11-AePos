                       ...
                       PPMThread* thread, int numThread, cudaStream_t* streamSmooth, int filtro)
{
    ...
    gpuErrchk( cudaMemcpy2DAsync(GPU_input,gpu_image_pitch,CPUinput,widthStep,width,thread[numThread].linhas-200,cudaMemcpyHostToDevice, streamSmooth[numThread]) );
